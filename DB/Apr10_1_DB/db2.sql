...
select 1 id, '수학' subject , 90 score from dual
	union
select 1 id, '수학' subject , 90 score from dual; -- 중복된 값이라 하나만 출력된다.
...
-- UNION ALL : 중복되더라도 모두 출력
 ...
 select 1 id, '수학' subject , 90 score from dual
 	union all
 select 1 id, '수학' subject , 90 score from dual; -- 중복된 값이라도 전부 출력된다.
...
select * from apr06_snack where s_maker = '크라운' and (s_price <= 1000 or s_price >= 3000);

-- GROUP BY : 그룹으로 묶여 연산해서 하나의 결과를 나타내는 것 
--	회사별 과자, 평균가격, 최소가격, 최고가격 구하기

--	GROUP BY 사용 전 : 최고가, 최소가, 평균가 -> 구할 수 없다.
SELECT AVG(S_PRICE), MIN(S_PRICE), MAX(S_PRICE) FROM APR06_SNACK; 
-- 회사별 조건이 아닌 테이블 전체의 값이 나옴

-- group by를 이용해서 회사별로 그룹을 지으면
SELECT S_MAKER, AVG(S_PRICE), MIN(S_PRICE), MAX(S_PRICE) 
	FROM APR06_SNACK
	GROUP BY S_MAKER;
...
select s_name, s_price from apr06_snack where s_price < (select avg(s_price) from apr06_snack);
...
select s_name, s_maker, s_price from apr06_snack where s_price = (select max(s_price) from apr06_snack);
-- 제일 싼 과자는 어디서 만드나요?
select s_maker from apr06_snack where s_price = (select max(s_price) from apr06_snack);
-- 평균가보다 비싼 과자는 몇 종류
select count(*) from apr06_snack where s_price > (select avg(s_price) from apr06_snack);
-- 유통기한이 가장 오래 남은 과자의 전체 정보 조회
select * from apr06_snack where s_exp = (select max(s_exp) from apr06_snack);
----------------------------------------------------------------------------------------------------
