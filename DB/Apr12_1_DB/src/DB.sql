...
select *
	from (
		select rownum rn, c_no, c_name, c_os, c_fomat_date, c_weight, c_cpu
		from (
			-- 가장 가벼운 컴퓨터의 전체정보
			select * 
			from apr12_computer 
...
-- 이마트라는 단어가 들어간 매장은 모두 가격을 20% 할인
...
delete  from apr12_product where p_name = '%배추%'
