...
alter table apr05_notebook modify n_ram number(3);

-- 예제3)
-- 노트북 크기에 대한 컬럼을 추가하는 문장
-- 	[단위 cm, 소수점 두번째 자리까지 나타낼 것]
--  [이 컬럼은 빈 값으로 넣을 수 없음]
alter table apr05_notebook add n_size number(5,2) not null;

-- 예제4)
-- 노트북에 앞으로 hdd 대신 ssd가 장착
alter table apr05_notebook rename column n_hdd to n_ssd;

-- 예제5)
-- 보증기간 컬럼을 삭제하는 문장 작성
alter table apr05_notebook drop column n_warranty;

-- 예제6)
-- 노트북의 가격이 [800000 ~2300000] 사이의 값만 들어갈 수 있게
--	해당 컬럼을 변경하는 문장 작성
alter table apr05_notebook modify n_price number(7) 
	constraint n_check check(n_price between 800000 and 2300000);
	
-- 예제7)
--	만든테이블을 완전히 삭제하는 문장 작성
drop table apr05_notebook cascade constraint purge;
...
insert into apr05_snack(s_no, s_name, s_company, s_price) values(1,'초코파이','오리온',5000);

-- 입력할 때 순서 바꾸기? -> 가능
insert into apr05_snack(s_name, s_price, s_no, s_company) values('초코파이',3500,2,'롯데');

-- 컬럼에 값을 안넣기 -> error not null 때문에 
-- 만약에 null이 허용되는 경우 였으면 null이 가능한 컬럼을 빼고 
-- 값을 입력했어도 정상적으로 들어갔을 것
insert into apr05_snack(s_no, s_name) values(3,'마이쮸') -- null이었으면 가능했을 것 

-- (컬럼명, 컬럼명, ..) 넣지 않고 컬럼 순서대로 넣는방법!
insert into apr05_snack values(4,'엄마손파이','롯데',4000);

select * from apr05_snack;
------------------------------------------------------------------------------------------------
-- 테이블 지우고
-- 다시 테이블 생성해서 값 5개 정도만 넣기 
drop table apr05_snack cascade constraint purge;

insert into apr05_snack values(1,'초코파이','롯데',5000);
insert into apr05_snack values(2,'초코파이','오리온',5000);
insert into apr05_snack values(3,'마이쮸','롯데',500);
insert into apr05_snack values(4,'허니버터칩','해태',3000);
insert into apr05_snack values(5,'꼬북칩','크라운',2000);
...
create sequence apr05_snack_seq;
