...
select max(f_seat) from apr11_franchise;

select f_ceo from apr11_franchise
	where f_seat = (
		select max(f_seat) 
...
select rownum, m_no, m_name, m_price, m_f_no 
	from apr11_menu 
	order by m_name asc;
	where rownum >= 1 and rownum <= 3 -- 문법 상 오류
	
select rownum, m_no, m_name, m_price -- 2. 그 후에 rownum 부여
	from(
	--  1. 이 부분을 테이블처럼 사용(데이터 먼저 정렬해 놓은 후에)
		select m_no, m_name, m_price, m_f_no
		from apr11_menu order by m_name -- Inline View라고 함(인라인 뷰)				
		)
		where rownum >= 1 and rownum <= 3; 
...
-- EQUI 조인 : JOIN ~ USING(조인대상 컬럼명)
select * from tb1 join tb2 using (name); 
select * from tb1 natural join tb2; -- 자연조인(Natural join) : 조건절 없이 양쪽에 같은 이름을 가진 동일한 컬럼만 조회
...
select A.m_id, A.m_name, B.m_id
		-- 같은 테이블이지만 셀프조인을 하기 위해 테이블 별로 별칭을 넣어서 구분
